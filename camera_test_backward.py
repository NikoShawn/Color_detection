...
class ColorDetection:
    ...
    def transform_to_world(self, camera_coords):
        ...
        try:
            ...
            x_local = camera_coords[0] # 相机坐标系下的X (通常是右)
            # y_local = camera_coords[1] # 相机坐标系下的Y (通常是下)
            z_local = -camera_coords[2] # 相机坐标系下的Z (通常是前/深度)

            # 如果摄像头朝向与机器人相反:
            # 物体在相机前方 z_local 距离，意味着在机器人后方 z_local 距离。
            # 物体在相机右方 x_local 距离，假设对应机器人左方 x_local 距离。
            # (如果相机右方对应机器人右方，则 x_local 前的符号可能需要调整)
            
            # 机器人前方为X轴正方向，左方为Y轴正方向
            # world_x_offset = -z_local * np.cos(yaw) - x_local * np.sin(yaw)
            # world_y_offset = -z_local * np.sin(yaw) + x_local * np.cos(yaw)
            
            # world_x = robot_x + world_x_offset
            # world_y = robot_y + world_y_offset
            # world_z = robot_z # 保持Z坐标不变，或根据需要调整

            # 更新后的转换公式
            world_x = robot_x - z_local * np.cos(yaw) - x_local * np.sin(yaw)
            world_y = robot_y - z_local * np.sin(yaw) + x_local * np.cos(yaw)
            world_z = robot_z # Z坐标通常保持不变，或根据相机相对于机器人基座的垂直位移调整
